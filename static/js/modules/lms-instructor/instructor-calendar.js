...
// --- DYNAMIC ANNOUNCEMENTS FETCH & ACCORDION LOGIC ---
async function fetchAnnouncements() {
    const announcementsList = document.querySelector('.announcements-list');
    
    try {
        // Fetch data from your new separate Flask API (running on port 5001)
        ...
        if (announcements.length === 0) {
            announcementsList.innerHTML = '<p style="padding: 15px; color: #666; font-size: 13px;">No announcements at this time.</p>';
            return;
        }
        
        // Loop through the database results and create the HTML structure
        announcements.forEach(announcement => {
            ...
            const accordionHTML = `
                <div class="announcement-accordion">
                    <button class="accordion-header">
                        ${announcement.event_name} <span class="accordion-icon">&#9660;</span>
                    </button>
            ...
            announcementsList.insertAdjacentHTML('beforeend', accordionHTML);
        });

        // Attach click events to the newly generated accordions
        attachAccordionLogic();

    } catch (error) {
        console.error("Could not fetch announcements:", error);
        announcementsList.innerHTML = '<p style="padding: 15px; color: #cc0000; font-size: 13px;">Failed to load announcements from server.</p>';
    }
}
...
// Fetch the announcements as soon as the DOM loads
document.addEventListener("DOMContentLoaded", () => {
    fetchAnnouncements();
});

// --- TOAST ENGINE (Integrates with your existing CSS) ---
function showToast(message, type = 'success') {
    ...
    // Apply the base class and the specific type (success, error, or info)
    toast.className = `toast toast-${type}`;
    toast.textContent = message;

    container.appendChild(toast);

    // 3. Trigger the entrance animation (toast-show)
    // A small delay (10ms) ensures the transition runs correctly after being appended to the DOM
    setTimeout(() => {
    ...
    setTimeout(() => {
        ...
        // Wait for the CSS transition to finish before completely removing from the DOM
        setTimeout(() => {
            toast.remove();
        }, 250);
    }, 3000);
